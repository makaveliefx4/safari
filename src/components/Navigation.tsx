...
import { Menu, X, Compass, MapPin, Camera, Bed, Phone, Info } from 'lucide-react';
...
export function Navigation() {
	...
	return (
		<>
			...
			<div
				className={cn(
					'fixed top-2 right-2 z-50 transition-all duration-300',
			...
			>
				<Button
					...
					onClick={toggleMenu}
					className={cn(
						"hover:bg-transparent md:w-auto group px-8 py-4 rounded-md shadow-lg border-border",
						isScrolled ? "bg-black/20 text-white" : "bg-transparent text-gray-800"
					)}
				>
					{/* Custom hamburger icon container */}
					...
					{navItems.map(({ to, label, icon: Icon }, index) => (
						...
						className={({ isActive }) =>
							...
							isActive
								...
								: 'text-foreground hover:bg-white hover:text-accent-foreground'
							)
						}
